...
export default function Layout({ children, language = 'en', onToggleLanguage }) {
  ...
  const useHeroHeader = isHome && !scrolled && !open;
    ...
    <div className="min-h-screen bg-[#f7fbfb] text-stone-950">
      <header
        className={`fixed left-0 right-0 top-0 z-50 transition-all duration-500 ${
          useHeroHeader ? 'bg-transparent' : 'border-b border-stone-200 bg-white/95 shadow-sm backdrop-blur-xl'
        }`}
      >
        <nav className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6">
            ...
            <div>
              <p
                className={`brand-name text-lg font-bold leading-tight transition-colors ${
                  useHeroHeader ? 'text-enamel' : 'text-teal-950'
                }`}
              >
                {clinicContact.name}
              </p>
              <p className={`text-xs ${useHeroHeader ? 'text-white/70' : 'text-slate-500'}`}>
                {clinicContact.label}
              </p>
            </div>
          </Link>
          ...
          <button
            ...
            className={`inline-flex h-10 w-10 items-center justify-center rounded-md border md:hidden ${
              useHeroHeader ? 'border-white/25 text-white' : 'border-stone-200 text-stone-900'
            }`}
